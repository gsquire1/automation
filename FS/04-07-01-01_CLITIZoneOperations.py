...
def parse_args(args):
    ...
    #parser.add_argument('-z',   '--zone_name_size', type=int, default=5, help="number of alphanumeric characters in zonename")
    ...
    #zonenamesize = args.zone_name_size
...
#######################################################################################################################
    ...
    f = "%s%s%s%s"%("logs/CLITIZoneOperations_test_case_file","_"+ipaddr_switch+"_",date_is,".txt")
...
#############################################################################################
    ...
    ti_zonename = (''.join(random.choice(string.ascii_letters + string.digits + character) for _ in range(size)) + "_TI")
