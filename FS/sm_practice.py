...
###############################################################################
    ...
    def get_wwn(self,  fid = -1):
        ...
        """
        sm = requests.get("http://10.38.36.33/rest/running/switch/fibrechannel-switch", headers=self.Auth)
        print(sm.json)
        print(sm.text)
        print(sm.headers)
        ras = re.compile('name>([\:0-9a-f]+)</n')
        ras = ras.findall(sm.text)
        print(type(ras))
        print(ras)
        wwn = ras[0]
        print(type(wwn))
...
def main():
    ...
    group = "all"           #### the group to get the information for 
    print("="*80)
...
#################
    ...
    print(Auth_send)
    #r = requests.get("http://%s/rest/running/switch/fibrechannel-switch/name"  % ( pa.ip, wwn) , headers=Auth
    r = requests.get("http://%s/rest/running/switch/fibrechannel-switch/name/%s/up-time" % (pa.ip, wwn) , headers=Auth_send)
    print('((((((((((((((((((((((((UPTIME))))))))))))))))))))))))')
    print("@"*80)
    print(r.text)
    print("@"*80)
